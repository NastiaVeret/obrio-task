...
def _reviews_feed_url(country: str, app_id: str, page: int) -> str:
    """Build Apple customerreviews RSS URL.

    Apple's feed is inconsistent: for some apps (including Nebula),
    ``page=1`` returns an empty feed while the same URL without a page
    segment returns reviews. Later pages still need ``page=N``.
    """
    base = f"https://itunes.apple.com/{country}/rss/customerreviews"
    if page <= 1:
        return f"{base}/id={app_id}/sortBy=mostRecent/json"
    return f"{base}/page={page}/id={app_id}/sortBy=mostRecent/json"
...
def collect_app_store_reviews(
    ...
    # Apple sometimes returns empty pages interleaved with valid ones.
    empty_streak = 0
    ...
    try:
        with httpx.Client(
            ...
            for page in range(1, MAX_PAGES + 1):
                url = _reviews_feed_url(country, app_id, page)
                try:
                ...
                if not page_reviews:
                    empty_streak += 1
                    # Stop after a few consecutive empties; Apple's feed
                    # occasionally blanks a middle page but still serves later ones.
                    if empty_streak >= 3:
                        break
                    continue
                empty_streak = 0
